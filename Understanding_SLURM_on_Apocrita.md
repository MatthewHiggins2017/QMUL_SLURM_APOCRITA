...
- **Running analyses in parallel** – multiple independent tasks run simultaneously
- **Breaking problems into pieces** – allowing many separate compute cores (CPUs) to work on the same problem at the same time
...
1. **Resource allocation** – allocates exclusive and/or non-exclusive access to compute resources (nodes) to users for a duration of time
2. **Job scheduling** – provides a framework for starting, executing, and monitoring work (typically batch jobs) on the set of allocated nodes
3. **Queue management** – arbitrates contention for resources by managing a queue of pending jobs
...
| ddy | – | 167 | 48 | 384GB | Standard compute |
| sbg (ampere) | `--gres=gpu:X` | 1 | 48 | 384GB | GPU nodes |
| sbg (volta) | `--gres=gpu:X` | 2 | 32 | 384GB | GPU nodes |
| sdx | – | 52 | 36 | 384GB | Standard compute |
| smf | – | 8 | 24 | 768GB | High memory |
| srm | – | 2 | 36 | 768GB | High memory |
| vhm | – | 4 | 64 | 3TB | Very high memory |

> **Note:** Hyperthreading is not enabled across the cluster. Some nodes are restricted depending on the source of funding.

📖 **Documentation:** https://slurm-docs.hpc.qmul.ac.uk/nodes/
...
⚠️ **Warning:** Avoid using your home directory as a destination for data generated by cluster jobs, as home directories fill up quickly and **exceeding your quota will cause your jobs to fail**.
...
- **Do not abuse this service** by modifying timestamps to make it permanent storage – you may lose access

📖 **Documentation:** https://slurm-docs.hpc.qmul.ac.uk/storage/scratch/
...
- `app/1.1.0-gcc-12.2.0` – version 1.1.0 compiled with GCC 12.2
- `app/1.1.0-openmpi-5.0.3-gcc-12.2.0` – includes OpenMPI 5.0.3
- `app/1.1.0-openmpi-5.0.3-cuda-12.4.0-gcc-12.2.0` – includes CUDA 12.4
...
📖 **Documentation:** https://slurm-docs.hpc.qmul.ac.uk/using/UsingModules/
...
⚠️ **Important:** Use `#SBATCH` (with the letter "S"), not `#$BATCH` (with a dollar symbol).
...
📖 **Documentation:** https://slurm-docs.hpc.qmul.ac.uk/using/uge_to_slurm/
...
Array jobs allow you to submit many similar jobs with a single script – extremely useful in bioinformatics for processing multiple samples.
...
⚠️ **Note:** GPU access is not permitted for Undergraduate/MSc students or external users.
